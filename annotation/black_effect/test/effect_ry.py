...
class TestRyEffect(tf.test.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_ry_effect(self):
        ...
        ry_effect = BlackRyEffectLayer(
            data_format=self.data_format, use_cudnn=self.use_cudnn
        )(ph, available_board)
        # 余計な方向がないことを確認する
        self.assertEqual(8, len(ry_effect))
        # 利用しやすいように次元を下げる
        ry_effect = {direction: tf.squeeze(effect)
                     for direction, effect in ry_effect.items()}
        # 8種類の距離を一つにまとめる
        cross_directions = get_cross_directions()
        ry_effect = {
            direction: tf.reduce_any(effect, axis=0)
            if direction in cross_directions else effect
            for direction, effect in ry_effect.items()
        }
        ...
        with self.test_session() as sess:
            for i, j in product(range(9), repeat=2):
                ...
                effect = sess.run(ry_effect, feed_dict={ph: board})
        ...
        with self.test_session() as sess:
            for i, j in product(range(9), repeat=2):
                ...
                for pin_direction in PinDirection:
                    ...
                    if self.data_format == 'NCHW':
                        board[0, 0, i, j] = Piece.BLACK_RY + offset
                    else:
                        board[0, i, j, 0] = Piece.BLACK_RY + offset
