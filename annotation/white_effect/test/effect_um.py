        ...
        with self.test_session() as sess:
            for i, j in product(range(9), repeat=2):
                ...
                if self.data_format == 'NCHW':
                    board[0, 0, i, j] = Piece.WHITE_UM
                else:
                    board[0, i, j, 0] = Piece.WHITE_UM
                    ...
                    for direction in get_eight_directions():
                        ...
                        for x, y in zip(s, t):
                            if x in range(9) and y in range(9):
                                self.assertEqual(count[x, y], 1, msg=(x, y))
                                c += 1
        ...
        with self.test_session() as sess:
            for i, j in product(range(9), repeat=2):
                ...
                for pin_direction in PinDirection:
                    ...
                    if self.data_format == 'NCHW':
                        board[0, 0, i, j] = Piece.WHITE_UM + offset
                    else:
                        board[0, i, j, 0] = Piece.WHITE_UM + offset
